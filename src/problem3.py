...
def run_test_problem3a():
    ...
    title += '  Test 5: Start at (20, 60), 4 lines.'
...
def problem3a(window, point, n):
    ...
    frompoint = rg.Point(point.x, point.y + 50)

    thickness = 0
    thick = 1
    for k in range(n):
        line = rg.Line(point, frompoint)
        ...
        point.x = point.x + 20
        point.y = point.y + 10
        frompoint.x = frompoint.x + 20
        frompoint.y = frompoint.y + 10
...
def problem3b(m, point1):
    ...
    for k in range(m):
        ...
        thick = problem3a(window, point, 3 + (2 * k))
